...
Return the public result envelope from `references/input_and_evidence_protocol.md`. Use the Notes diagnostic only as an internal readiness state, and declare `artifact_generated` and `docx` only after the requested file exists and passes its source, structure, and rendered-page checks.
